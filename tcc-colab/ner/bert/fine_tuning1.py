...
from ner.bert.bert_utils import pre_processar_tokens
...
def treinar():
    ...
    configuracao = AutoConfig.from_pretrained("neuralmind/bert-base-portuguese-cased", id2label=id2tag,
                                              label2id=tag2id)
...
def __processar_base():
    ...
    train_encodings = tokenizer(train_texts, return_offsets_mapping=True, padding=True,
                                truncation=True, is_split_into_words=True)
    val_encodings = tokenizer(val_texts, return_offsets_mapping=True, padding=True,
                              truncation=True,is_split_into_words=True)
...
def __get_textos_tags():
    ...
    with jsonlines.open("../../dados/labeled_4_labels.jsonl") as reader:
        for obj in reader:
            texto = obj['text']
            labels = obj['labels']
